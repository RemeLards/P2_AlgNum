...
x = [0, 0.8975979, 1.7951958, 2.692794, 3.5903916, 4.48798951, 5.3855874, 6.2831853] # Pontos Dados

y = [1, 0.52103, -0.15540, -0.52580, -0.43939, -0.09069, 0.21235, 0.28461] # Valores Dados

f = CubicSpline(x, y, bc_type='natural') # Modelando como um Spline Natural
...
original_function_value = math.exp(-0.2 * (x_new[number3_index]))*math.cos(x_new[number3_index]) # Valor Da Função Original no ponto mais próximo x = 3

error_percentage = 100* (1-(original_function_value/y_new[number3_index])) # Erro Percentual neste ponto mais próximo de 3
...
dydx = np.gradient(y_new,x_new) # Cálculo Primeira Derivada
...
d2ydx2 = np.gradient(dydx, x_new) # Cálculo Segunda Derivada
...
# Valor Da Função Original no ponto mais próximo x = 3.59

error_percentage2 =  (1 -(y_new[x_index_for_derivative]/original_function_value2) )*100
# Erro Percentual neste ponto mais próximo de 3.59
